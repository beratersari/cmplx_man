...
import ReservationCategoryForm from './ReservationCategoryForm';
import {
...
const ReservationCategoryManagement: React.FC = () => {
  ...
  const { data: categories, isLoading: categoriesLoading, refetch } = useGetReservationCategoriesQuery({
    complexId: complexFilter ? Number(complexFilter) : undefined,
  });
  const [createCategory, { isLoading: isCreating }] = useAdminCreateReservationCategoryMutation();
  const [updateCategory, { isLoading: isUpdating }] = useAdminUpdateReservationCategoryMutation();
  const [deleteCategory, { isLoading: isDeleting }] = useDeleteReservationCategoryMutation();
  ...
  const openEditModal = (category: ReservationCategory) => {
    setSelectedCategory(category);
    setIsEditModalOpen(true);
  };

  const openDeleteModal = (category: ReservationCategory) => {
    setSelectedCategory(category);
    setDeleteError(null);
  ...
  // Table columns
  const columns: Column<ReservationCategory>[] = [
    {
      key: 'id',
      header: 'ID',
      className: 'w-16',
...
export default ReservationCategoryManagement;
