...
from app.repositories import ReservationCategoryRepository, ComplexRepository
from app.models.models import UserModel, ReservationCategoryModel
from app.core.entities import UserRole
from app.api.v1.schemas import ReservationCategoryCreate, AdminReservationCategoryCreate, ReservationCategoryUpdate
from app.core.logging_config import logger


class ReservationCategoryService:
    """Service for reservation category-related business logic."""
    ...
    def create_category_in_my_complex(
    ...
    ) -> ReservationCategoryModel:
        """
        Create a new category in the manager's complex.
        Complex ID is extracted from the authenticated manager's context.
        """
        logger.info(f"Manager {current_user.username} (ID: {current_user.id}) creating reservation category: {category_in.name}")
        ...
        if self.category_repo.category_exists_in_complex(category_in.name, complex_obj.id):
            ...
            raise HTTPException(status_code=400, detail="Reservation category already exists in this complex")
        ...
        logger.info(f"Reservation category created successfully: {new_category.name} (ID: {new_category.id}) in complex ID {complex_obj.id}")
        return new_category
    ...
    def admin_create_category(
        ...
        """
        logger.info(f"Admin {current_user.username} (ID: {current_user.id}) creating reservation category: {category_in.name} for complex ID {category_in.complex_id}")
        ...
        new_category = self.category_repo.create(category_data, created_by=current_user.id)
        logger.info(f"Reservation category created successfully: {new_category.name} (ID: {new_category.id})")
        return new_category
    
    def get_category_by_id(self, category_id: int) -> ReservationCategoryModel:
    ...
    def list_categories(
        ...
        logger.trace(f"User {current_user.username} (ID: {current_user.id}) fetching reservation categories")
    ...
    def update_my_category(
        ...
        """
        logger.info(f"Manager {current_user.username} (ID: {current_user.id}) updating reservation category ID {category_id}")
        ...
        if category_in.name and self.category_repo.category_exists_in_complex(
            ...
            raise HTTPException(status_code=400, detail="Reservation category with this name already exists in this complex")
        ...
        logger.info(f"Reservation category ID {category_id} updated successfully by {current_user.username}")
        return category
    ...
    def admin_update_category(
        ...
        """
        logger.info(f"Admin {current_user.username} (ID: {current_user.id}) updating reservation category ID {category_id}")
    ...
    def delete_category(self, category_id: int, current_user: UserModel) -> dict:
        ...
        logger.info(f"User {current_user.username} (ID: {current_user.id}) deleting reservation category ID {category_id}")
        ...
        logger.info(f"Reservation category ID {category_id} deleted successfully by {current_user.username}")
        return {"message": "Reservation category deleted successfully"}
    
    # Private validation methods
    def _validate_modify_permission(self, current_user: UserModel, category: ReservationCategoryModel):
        ...
        logger.warning(f"Unauthorized update attempt by {current_user.username} (not a manager)")
        raise HTTPException(status_code=403, detail="Only managers can update reservation categories")
