...
from app.repositories import MarketplaceCategoryRepository, ComplexRepository
from app.models.models import UserModel, MarketplaceCategoryModel
from app.core.entities import UserRole
from app.api.v1.schemas import MarketplaceCategoryCreate, AdminMarketplaceCategoryCreate, MarketplaceCategoryUpdate
from app.core.logging_config import logger


class MarketplaceCategoryService:
    """Service for marketplace category-related business logic."""
    ...
    def create_category_in_my_complex(
        ...
        """
        logger.info(f"Manager {current_user.username} (ID: {current_user.id}) creating marketplace category: {category_in.name}")
        
        # Validate that user is a manager, attendant, or admin
        if current_user.role not in [UserRole.ADMIN, UserRole.SITE_MANAGER, UserRole.SITE_ATTENDANT]:
            logger.warning(f"Unauthorized category creation attempt by {current_user.username}")
            raise HTTPException(status_code=403, detail="Only admins, site managers, or site attendants can create marketplace categories")
        ...
        logger.info(f"Marketplace category created successfully: {new_category.name} (ID: {new_category.id}) in complex ID {complex_obj.id}")
        return new_category
    ...
    def admin_create_category(
        ...
        """
        logger.info(f"Admin {current_user.username} (ID: {current_user.id}) creating marketplace category: {category_in.name} for complex ID {category_in.complex_id}")
        ...
        new_category = self.category_repo.create(category_data, created_by=current_user.id)
        logger.info(f"Marketplace category created successfully: {new_category.name} (ID: {new_category.id})")
        return new_category
    
    def get_category_by_id(self, category_id: int) -> MarketplaceCategoryModel:
    ...
    def list_categories(
        ...
        logger.trace(f"User {current_user.username} (ID: {current_user.id}) fetching marketplace categories")
        ...
        # For managers, attendants, and residents, return categories in their complex
        if current_user.assigned_complexes:
            complex_obj = current_user.assigned_complexes[0]
            return self.category_repo.get_categories_by_complex(complex_obj.id, skip, limit)
        
        # For residents assigned to buildings, get the complex from their building
    ...
    def update_my_category(
        ...
        """
        logger.info(f"Manager {current_user.username} (ID: {current_user.id}) updating marketplace category ID {category_id}")
        
        # Validate that user is a manager, attendant, or admin
        if current_user.role not in [UserRole.ADMIN, UserRole.SITE_MANAGER, UserRole.SITE_ATTENDANT]:
            logger.warning(f"Unauthorized update attempt by {current_user.username}")
            raise HTTPException(status_code=403, detail="Only admins, site managers, or site attendants can update marketplace categories")
        ...
        # Validate that category belongs to user's complex (unless admin)
        if current_user.role != UserRole.ADMIN and category.complex_id != user_complex.id:
        ...
        if category_in.name and self.category_repo.category_exists_in_complex(
            category_in.name, category.complex_id, exclude_id=category_id
        ):
            logger.warning(f"Update failed: Category name {category_in.name} already exists in complex ID {category.complex_id}")
        ...
        logger.info(f"Marketplace category ID {category_id} updated successfully by {current_user.username}")
        return category
    ...
    def admin_update_category(
        ...
        """
        logger.info(f"Admin {current_user.username} (ID: {current_user.id}) updating marketplace category ID {category_id}")
    ...
    def delete_category(self, category_id: int, current_user: UserModel) -> dict:
        ...
        logger.info(f"User {current_user.username} (ID: {current_user.id}) deleting marketplace category ID {category_id}")
        ...
        logger.info(f"Marketplace category ID {category_id} deleted successfully by {current_user.username}")
        return {"message": "Category deleted successfully"}
    
    # Private validation methods
    def _validate_modify_permission(self, current_user: UserModel, category: MarketplaceCategoryModel):
        ...
        logger.warning(f"Unauthorized update attempt by {current_user.username}")
        raise HTTPException(status_code=403, detail="Only admins, site managers, or site attendants can modify marketplace categories")